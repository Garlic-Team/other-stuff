...
const checkValidation = async (arg, content, client, guild, argument, channel, user) => {
    if (!content) {
        const text = (await Util_1.Util.getResponse('ARGUMENT_REQUIRED', { client })).replace('{user}', user.toString()).replace('{name}', argument.name).replace('{type}', Util_1.Util.toPascalCase(Argument_1.ArgumentType[argument.type.toString()]));
        if (argument.type === Argument_1.ArgumentType.STRING && argument.choices && argument.choices.length !== 0) {
            ...
            const component = await channel.awaitMessageComponent({ filter: (m) => m.componentType === 'SELECT_MENU' && m.user.id === user.id && m.channelId === channel.id && m.message.id === message.id && m.customId === 'argument_choices', time: 60000 });
    ...
    if (command.cooldown) {
        ...
        if (cooldown)
            return message.reply({
                content: (await Util_1.Util.getResponse('COOLDOWN', { client })).replace('{time}', String(cooldown)).replace('{name}', command.name + ' command'),
            });
    }
