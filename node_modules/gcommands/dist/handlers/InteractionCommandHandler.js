...
async function InteractionCommandHandler(interaction) {
    ...
    if (!command && client.options?.unknownCommandMessage)
        return interaction.reply({
            content: (await Util_1.Util.getResponse('NOT_FOUND', interaction)),
            ephemeral: true,
        });
    if (command.cooldown) {
        ...
        if (cooldown)
            return interaction.reply({
                content: (await Util_1.Util.getResponse('COOLDOWN', interaction)).replace('{time}', String(cooldown)).replace('{name}', command.name + ' command'),
                ephemeral: true,
            });
    }
