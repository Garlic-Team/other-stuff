const { MessageSelectMenu, MessageSelectMenuOption, MessageActionRow, MessageButton, Command } = require("gcommands")
...
module.exports = class extends Command {
    ...
    async run({client, respond, edit, member}) {
        let questionCounter = 0;
        let endCounter = 0;

        const dropdown = new MessageSelectMenu()
            .setCustomId("app_dropdown")
            .setMaxValues(1)
            .setMinValues(1)
            ...
            authorCollector.on("end", (collected, reason) => {
                if(reason == "succ") {
                    ...
                    let accept = new MessageButton()
                        .setLabel("Accept")
                        .setStyle("green")
                        .setCustomId(`accept_${collected.first().author.id}`)

                    let deny = new MessageButton()
                        .setLabel("Deny")
                        .setStyle("red")
                        .setCustomId(`deny_${collected.first().author.id}`)
